...
contract CrosschainRewardTransmitterAlter is IGelatoResolver, Owned {
    ...
    function checker() external view override returns (bool canExec, bytes memory execPayload) {
        // ensure time is slightly after the current epoch begins and we haven't executed for this epoch
        uint256 epoch = block.timestamp / (1 weeks);
        uint256 currentEpochStart = epoch * (1 weeks);
        if (block.timestamp > currentEpochStart + WINDOW || didTransmitForEpoch[epoch]) return (false, bytes(""));
        ...
        return (true, abi.encodeCall(CrosschainRewardTransmitterAlter.transmitMultiple, (gaugeList, cost)));
    }
